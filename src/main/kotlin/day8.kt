class Day8(input: String) {
    data class Number(val segment: String, var number: Int?) {

        fun missingChars(other: Number): Int = other.segment.fold(0) { acc, c ->
            if (segment.contains(c)) acc else acc + 1
        }
    ...
    fun day8Part1() =
        signalAndSegment.flatMap {
            it.second.map { findDigit(it) }
        }.filterNotNull().size
    ...
    private fun findDigitsForSignal(signal: String) =
        Number(
            signal, when (signal.length) {
                2 -> 1
                4 -> 4
                3 -> 7
                7 -> 8
    ...
    private fun findDigit(signal: String) =
        when (signal.length) {
            2 -> "cf"       //1
            4 -> "bcdf"     //4
            3 -> "acf"      //7
